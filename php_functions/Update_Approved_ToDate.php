...
// Obtener el parámetro 'id' desde la URL (por ejemplo, 'barcaza')
$seccion = isset($_GET['id']) ? $_GET['id'] : 'Barcazas';  
$seccion = isset($_GET['id']) ? $_GET['id'] : 'Capitanes';
$seccion = isset($_GET['id']) ? $_GET['id'] : 'Tanqueros';
$seccion = isset($_GET['id']) ? $_GET['id'] : 'Inspectores';
$seccion = isset($_GET['id']) ? $_GET['id'] : 'Cia_Inspeccion';
$seccion = isset($_GET['id']) ? $_GET['id'] : 'Remolcadores';
$seccion = isset($_GET['id']) ? $_GET['id'] : 'Pilotos';
...
$sql = "
    SELECT *
    FROM documentos
    WHERE categoria = '$seccion' AND status = 'Aprobado'
          ...
          <div class="col-md-12">
            <div class="card">
              <div class="card-header">
                <h4 class="card-title text-info"> <b> <?php echo ucfirst($seccion); ?></b> aprobados para operar en <b> MOTI </b> </h4>
                <a href="exportar_pdf.php?id=<?php echo urlencode($seccion); ?>" class="btn btn-primary">Descargar PDF</a>
              </div>
              <div class="card-body">
                <table class="table">
                  <thead>
                    <tr>
                      <th>NOMBRE DE <?php echo ucfirst($seccion); ?></th>
                      <th>CLIENTE</th>
                      <th>STATUS</th>
                    </tr>
                  </thead>
